...
export function TodayScreen() {
  ...
  const visibleCommitted = [...inSprint, ...outsideSprint]
  ...
  async function complete(action: GoalAction) {
    setBusy(action.id)
    try { await store.updateGoalAction(action.id, { status: 'completed' }); toast.success('Step completed') }
    catch (error) { toast.error(error instanceof Error ? error.message : 'Could not complete Step') }
    finally { setBusy(null) }
  }
  ...
  const rowProps = { busy, onStart: start, onComplete: complete, onResize: resize, onBacklog: moveToBacklog, onArchive: archive, onOpenGoal: openGoal }
  return (
    <>
...
function StepRow({ item: { goal, action }, busy, outsideSprint = false, onStart, onComplete, onResize, onBacklog, onArchive, onOpenGoal }: { item: StepItem; busy: string | null; outsideSprint?: boolean; onStart: (action: GoalAction) => void; onComplete: (action: GoalAction) => void; onResize: (action: GoalAction) => void; onBacklog: (action: GoalAction) => void; onArchive: (action: GoalAction) => void; onOpenGoal: (id: string) => void }) {
  const loggedMinutes = actionLoggedMinutes(action)
  return <div className="work-row flex flex-wrap items-center gap-2 py-3 first:pt-0 last:pb-0"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium leading-5">{action.title}</p><LedgerMeta className="mt-1 truncate">{goal.title} · {action.plannedMinutes}m{loggedMinutes > 0 ? ` · ${formatMinutes(loggedMinutes)} logged` : ''}{outsideSprint ? ' · Outside current Sprint' : ''}</LedgerMeta></div><div className="flex shrink-0 items-center gap-1"><Button size="sm" onClick={() => onStart(action)} disabled={busy === action.id}><Play className="h-3.5 w-3.5" /> Start</Button><Button size="sm" variant="outline" onClick={() => onComplete(action)} disabled={busy === action.id}><CheckCircle2 className="h-3.5 w-3.5" /> Complete</Button><DropdownMenu><DropdownMenuTrigger asChild><Button size="icon" variant="ghost" aria-label={`More options for ${action.title}`}><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onSelect={() => onOpenGoal(goal.id)}>Open Outcome</DropdownMenuItem><DropdownMenuItem onSelect={() => onResize(action)}><RotateCcw /> Resize Step</DropdownMenuItem><DropdownMenuItem onSelect={() => onBacklog(action)}><Clock3 /> Move to backlog</DropdownMenuItem><DropdownMenuSeparator /><DropdownMenuItem onSelect={() => onArchive(action)} className="text-red-400">Archive Step</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div></div>
}
