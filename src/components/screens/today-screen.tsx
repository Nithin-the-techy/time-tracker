  ...
  return (
    <LedgerPanel className="flex min-h-[calc(100dvh-10rem)] flex-col justify-center border-[var(--growth)]/60 shadow-[0_0_35px_rgba(251,191,36,0.06)]">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-[110px_1fr]">
        ...
        <div><Label htmlFor="result-note">Result note <span className="font-normal text-muted-foreground">(optional)</span></Label><Input id="result-note" value={output} onChange={(event) => setOutput(event.target.value)} placeholder="What changed, shipped, or became clear?" /></div>
      </div>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Button onClick={() => finish('completed')} disabled={finishing}><CheckCircle2 className="h-4 w-4" /> Finish focus block</Button>
        <Button variant="outline" onClick={() => finish('interrupted')} disabled={finishing}><Pause className="h-4 w-4" /> Pause and keep planned</Button>
        <Button variant="ghost" onClick={() => finish('abandoned')} disabled={finishing}><Clock3 className="h-4 w-4" /> Move to backlog</Button>
      </div>
    </LedgerPanel>
  )
}
