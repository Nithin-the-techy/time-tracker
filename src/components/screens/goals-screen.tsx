  ...
  return <div className="space-y-3">
    ...
    <LedgerPanel className="p-4">
      ...
      <div className="mt-4 space-y-4">
        ...
        <div className="grid items-start gap-4 sm:grid-cols-2"><Field label="Area"><select value={subdepartmentId} onChange={(event) => setSubdepartmentId(event.target.value)} className="h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm"><option value="">No category</option>{goal.department.subdepartments.map((sub) => <option key={sub.id} value={sub.id}>{sub.name}</option>)}</select></Field><Field label="Due"><Input type="date" value={dueDate} onChange={(event) => setDueDate(event.target.value)} /></Field></div>
        {hasStructure && <div className="grid items-start gap-4 rounded-sm border border-border/70 bg-background/20 p-3 sm:grid-cols-2"><Field label="Measure"><select value={targetId} onChange={(event) => setTargetId(event.target.value)} className="h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm"><option value="">No measure</option>{goal.targets.map((target) => <option key={target.id} value={target.id}>{target.label}</option>)}</select></Field><Field label="Blocker"><select value={problemId} onChange={(event) => setProblemId(event.target.value)} className="h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm"><option value="">No blocker</option>{goal.problems.map((problem) => <option key={problem.id} value={problem.id}>{problem.statement}</option>)}</select></Field></div>}
