...
export function SprintPanel() {
  ...
  function resetNewOutcome() {
    setNewOutcomeTitle(''); setNewOutcomeDate(toKey(addDays(new Date(), 17))); setGoalId('')
  }
  ...
  async function createOutcome() {
    ...
    if (!editableSprint || !departmentId || !newOutcomeTitle.trim() || busy) return
    setBusy(true)
    try {
      await store.createGoal({ departmentId, title: newOutcomeTitle.trim(), outcome: newOutcomeTitle.trim(), startDate: toKey(new Date()), targetDate: newOutcomeDate, sprintId: editableSprint.id })
  ...
  return <section className="space-y-3" aria-labelledby="sprints-heading">
    <div className="flex items-end justify-between gap-4"><LedgerSectionLabel id="sprints-heading">Sprints</LedgerSectionLabel><Button variant="outline" size="sm" onClick={() => openEditor(null, true)}><Plus className="h-4 w-4" /> Add Sprint</Button></div>

    {active ? <button type="button" onClick={() => openEditor(active)} className="group flex w-full items-center gap-4 rounded-md border border-border border-l-2 border-l-[var(--growth)] bg-card/70 p-4 text-left transition hover:border-foreground/35 hover:-translate-y-px"><span className="min-w-0 flex-1"><span className="block text-xs font-medium text-[var(--growth)]">Current Sprint</span><span className="mt-1 block truncate text-base font-semibold">{active.name}</span>{active.phase && <span className="mt-1 block truncate text-sm text-muted-foreground">{active.phase}</span>}</span><span className="text-right"><span className="block text-sm tabular-nums text-foreground">{deadlineLabel(daysRemaining(active.endDate))}</span><span className="mt-1 block text-xs text-muted-foreground">{active.goals.length} outcome{active.goals.length === 1 ? '' : 's'}</span></span><ChevronRight className="h-4 w-4 shrink-0 text-muted-foreground transition group-hover:translate-x-0.5" /></button> : <button type="button" onClick={() => openEditor(null, true)} className="flex w-full items-center gap-4 rounded-md border border-dashed border-border p-4 text-left transition hover:border-foreground/35"><span><span className="block text-sm font-semibold">Create your first Sprint</span></span></button>}

    {visibleOtherSprints.length > 0 && <details className="rounded-md border border-border/70 px-3 py-2"><summary className="cursor-pointer text-xs font-medium text-muted-foreground">Other Sprints</summary><div className="mt-2 divide-y divide-border/70">{visibleOtherSprints.map((sprint) => <div key={sprint.id} className="flex items-center gap-3 py-3"><button type="button" onClick={() => openEditor(sprint)} className="min-w-0 flex-1 text-left"><span className="block truncate text-sm font-medium">{sprint.name}</span><span className="mt-1 block text-xs text-muted-foreground">{sprintStatusLabel(sprint.status)} · {sprint.goals.length} outcome{sprint.goals.length === 1 ? '' : 's'}</span></button>{(['planned', 'paused'].includes(sprint.status)) && <Button size="sm" variant="ghost" onClick={() => setStatus(sprint.id, 'active')}><Play className="h-3.5 w-3.5" /> Activate</Button>}</div>)}</div></details>}

    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-h-[min(88dvh,780px)] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>{creating ? 'Add a Sprint' : `Edit ${editableSprint?.name ?? 'Sprint'}`}</DialogTitle><DialogDescription>{creating ? 'Name and dates are enough. Add a focus if useful.' : 'Edit the window or manage its outcomes.'}</DialogDescription></DialogHeader><div className="space-y-5"><div className="grid items-start gap-4 sm:grid-cols-2"><div className="sm:col-span-2"><Label>Name</Label><Input value={name} onChange={(event) => setName(event.target.value)} placeholder="September execution window" /></div><div className="sm:col-span-2"><Label>Focus <span className="font-normal text-muted-foreground">(optional)</span></Label><Input value={phase} onChange={(event) => setPhase(event.target.value)} placeholder="What is this Sprint for?" /></div><div><Label>Starts</Label><Input type="date" value={startDate} onChange={(event) => setStartDate(event.target.value)} /></div><div><Label>Ends</Label><Input type="date" min={startDate} value={endDate} onChange={(event) => setEndDate(event.target.value)} /></div></div>
      {editableSprint && <><div className="border-t border-border pt-5"><div className="flex items-baseline justify-between gap-3"><Label>Outcomes in this Sprint</Label><span className="text-xs tabular-nums text-muted-foreground">{editableSprint.goals.length} linked</span></div>{editableSprint.goals.length > 0 && <div className="mt-3 space-y-2">{editableSprint.goals.map((link) => <div key={link.goalId} className="rounded-sm border border-border/70 p-3"><div className="flex items-start justify-between gap-3"><span className="min-w-0 text-sm font-medium">{link.goal.title}</span><Button size="icon" variant="ghost" aria-label={`Remove ${link.goal.title} from Sprint`} onClick={() => detachGoal(link.goalId)} disabled={busy}><X className="h-4 w-4" /></Button></div>{movableSprints.length > 0 && <div className="mt-2 flex gap-2"><select aria-label={`Move ${link.goal.title} to another Sprint`} value={moveTargetByGoal[link.goalId] ?? ''} onChange={(event) => setMoveTargetByGoal((current) => ({ ...current, [link.goalId]: event.target.value }))} className="h-9 min-w-0 flex-1 rounded-md border border-input bg-transparent px-2 text-xs"><option value="">Move to another Sprint…</option>{movableSprints.map((sprint) => <option key={sprint.id} value={sprint.id}>{sprint.name} · {sprintStatusLabel(sprint.status)}</option>)}</select><Button size="sm" variant="outline" onClick={() => moveGoal(link.goalId)} disabled={!moveTargetByGoal[link.goalId] || busy}>Move</Button></div>}</div>)}</div>}{availableGoals.length > 0 && <div className="mt-3 flex gap-2"><select value={goalId} onChange={(event) => setGoalId(event.target.value)} className="h-10 min-w-0 flex-1 rounded-md border border-input bg-transparent px-3 text-sm"><option value="">Attach an existing outcome…</option>{availableGoals.map((goal) => <option key={goal.id} value={goal.id}>{goal.title}</option>)}</select><Button variant="outline" onClick={attachGoal} disabled={!goalId || busy}>Attach</Button></div>}</div><details className="rounded-md border border-border/70 p-3"><summary className="cursor-pointer text-sm font-medium">Add an outcome to this Sprint</summary>{creating ? <p className="mt-3 text-xs text-muted-foreground">Save the Sprint first, then add its first outcome here.</p> : <div className="mt-3 space-y-3"><div className="grid items-start gap-4 sm:grid-cols-2"><div><Label>Area</Label><select value={newOutcomeDepartmentId || departments[0]?.id || ''} onChange={(event) => setNewOutcomeDepartmentId(event.target.value)} className="h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm">{departments.map((department) => <option key={department.id} value={department.id}>{department.name.replace('Department of ', '')}</option>)}</select></div><div><Label>Due</Label><Input type="date" value={newOutcomeDate} onChange={(event) => setNewOutcomeDate(event.target.value)} /></div></div><div><Label>Outcome</Label><Input value={newOutcomeTitle} onChange={(event) => setNewOutcomeTitle(event.target.value)} placeholder="What result should exist by the end?" /></div><Button onClick={createOutcome} disabled={busy || !newOutcomeTitle.trim()}><Plus className="h-4 w-4" /> Add outcome</Button></div>}</details></>}
    </div><DialogFooter className="sm:justify-between">{!creating && editableSprint ? <div className="flex flex-wrap gap-1"><Button variant="ghost" onClick={() => setStatus(editableSprint.id, 'paused')}><Pause className="h-4 w-4" /> Put on hold</Button><Button variant="outline" onClick={() => setStatus(editableSprint.id, 'completed')}><Check className="h-4 w-4" /> Close Sprint</Button><Button variant="ghost" className="text-muted-foreground" onClick={() => { if (window.confirm('Archive this Sprint? Its outcomes stay available.')) setStatus(editableSprint.id, 'archived') }}>Archive</Button></div> : <span />}<Button onClick={save} disabled={busy || !name.trim()}>{busy ? 'Saving…' : creating ? 'Create Sprint' : 'Save changes'}</Button></DialogFooter></DialogContent></Dialog>
  </section>
}
