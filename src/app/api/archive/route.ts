...
export async function GET() {
  const [goals, actions, entries] = await Promise.all([
  ...
  return NextResponse.json({ goals, actions, entries })
}
...
export async function POST(req: NextRequest) {
  ...
  if (!id || !['goal', 'action', 'entry'].includes(entity)) return NextResponse.json({ error: 'entity and id are required' }, { status: 400 })
